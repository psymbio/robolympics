...
# import errors fix
from .robot_locomotors import Humanoid2
...
class WalkerBaseBulletEnv2(MJCFBaseBulletEnv):
  ...
  def __init__(self, robot, render=False):
    ...
    # changing walk walk_targets
    # not sure if we have a walk-target
    self.stateId = -1
    MJCFBaseBulletEnv.__init__(self, robot, render)

  def create_single_player_scene(self, bullet_client):
    # return here to create multiplayer scene
...
class HumanoidBulletEnv2(WalkerBaseBulletEnv2):
  ...
  def __init__(self, robot=None, render=False):
    if robot is None:
      self.robot = Humanoid2() # changed from Humanoid2
    else:
      self.robot = robot
      print("it didn't none")
    WalkerBaseBulletEnv2.__init__(self, self.robot, render)
    # WalkerBaseBulletEnv2.move_robot(self, 28, -20, 5)
