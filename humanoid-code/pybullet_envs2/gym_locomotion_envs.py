from .scene_stadium import SinglePlayerStadiumScene, SinglePlayerStadiumScene2
from .env_bases import MJCFBaseBulletEnv, URDFBaseBulletEnv
import numpy as np
import pybullet
...
class WalkerBaseBulletEnv2(MJCFBaseBulletEnv):
  ...
  def create_single_player_scene(self, bullet_client):
    self.stadium_scene = SinglePlayerStadiumScene2(bullet_client,
                                                  gravity=9.8,
                                                  timestep=0.0165 / 4,
                                                  frame_skip=4)
    return self.stadium_scene
  ...
  def reset(self):
    ...
    self.parts, self.jdict, self.ordered_joints, self.robot_body = self.robot.addToScene(
        self._p, self.stadium_scene.ground_plane_mjcf)
    # changed here
    # error: self.ground_ids = set([(self.parts[f].bodies[self.parts[f].bodyIndex], KeyError: 'floor'
...
class WalkerBaseBulletEnv3(URDFBaseBulletEnv):
...
class HumanoidBulletEnv2(WalkerBaseBulletEnv2):
  ...
  def __init__(self, robot=None, render=False):
    if robot is None:
      ...
      self.robot = robot
    WalkerBaseBulletEnv2.__init__(self, self.robot, render)
    self.electricity_cost = 4.25 * WalkerBaseBulletEnv2.electricity_cost
    self.stall_torque_cost = 4.25 * WalkerBaseBulletEnv2.stall_torque_cost

class HumanoidBulletEnv3(WalkerBaseBulletEnv3):

  def __init__(self, robot=None, render=False):
    if robot is None:
      self.robot = HumanoidURDF()
    else:
      self.robot = robot
    WalkerBaseBulletEnv3.__init__(self, self.robot, render)
    self.electricity_cost = 4.25 * WalkerBaseBulletEnv3.electricity_cost
    self.stall_torque_cost = 4.25 * WalkerBaseBulletEnv3.stall_torque_cost
