...
p.resetBasePositionAndOrientation(humanoid4, startLocations[0], [0, 0, 0, 1])
...
for j in range(p.getNumJoints(humanoid4)):
  ...
  p.changeVisualShape(humanoid4, j, rgbaColor=[1, 1, 1, 1])
  info = p.getJointInfo(humanoid4, j)
  #print(info)
...
p.changeVisualShape(humanoid4, 2, rgbaColor=[1, 0, 0, 1])
chest = 1
neck = 2
rightHip = 3
rightKnee = 4
...
while (p.isConnected()):
  ...
  if (useExplicitPD):
    ...
    basePos, baseOrn = p.getBasePositionAndOrientation(humanoid4)
    pose = [
